...
public class FtContentServiceImpl implements FtContentService {
    ...
    public FanXunResult updateContent(TbFtContent content) {
        try{
            TbFtContent result = contentMapper.selectByPrimaryKey(content.getCid());
            //验证用户的合法性，如果用户id与小程序id与文章中的用户id以及小程序id不一致的话就是非法用户
            if (!(result.getAppid() == content.getAppid() && result.getUid() == content.getUid())){
    ...
    public FanXunResult getContentByCid(Integer cid) {
        try {
            ...
            return FanXunResult.build(1000,"OK", cid);
        }catch (Exception e){
            return FanXunResult.build(3000,ExceptionUtil.getStackTrace(e));
        }
    }
    ...
    @Override
    public FanXunResult updateContentSelectd(Integer appId,Integer uid, Integer cid,Integer lastSelectedCid) {
        try{
